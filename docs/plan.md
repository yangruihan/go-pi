...
**任务清单：**
- [ ] Ollama 连接重试（指数退避，最多 3 次）
- [ ] 模型未加载时的友好提示（`ollama pull qwen2.5-coder:7b`）
- [ ] Bash 工具进程崩溃自动重启
- [ ] 会话文件写入失败的错误处理（不丢消息）
- [ ] `Ctrl+C` 正确退出（清理子进程、保存会话）

### 4.2 性能测试

**任务清单：**
- [ ] 测量：从启动到第一个 Token 的延迟（目标 < 1s）
- [ ] 测量：TUI 一帧渲染时间（目标 < 16ms）
- [ ] 测量：大会话（1000 条消息）的加载时间
- [ ] 识别并优化瓶颈

### 4.3 集成测试

**任务清单：**
- [ ] 端到端测试：启动 → 发送消息 → 工具调用 → 收到回复
- [ ] 会话持久化测试：保存 → 退出 → 恢复 → 验证消息一致
- [ ] 压缩测试：人工构造大会话，触发压缩，验证对话仍连贯
